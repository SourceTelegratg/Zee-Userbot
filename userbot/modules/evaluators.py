...
async def _(event):
    ...
    if len(final_output) > 4096:
        zed = final_output.replace("`", "").replace("**", "").replace("__", "")
        with io.BytesIO(str.encode(zed)) as out_file:
