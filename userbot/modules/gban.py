...
from userbot import DEVS, WHITELIST, blacklistzed
from userbot.events import register
from userbot.utils import chataction, get_user_from_event, zed_cmd
...
@zed_cmd(pattern="باند عام(?: |$)(.*)")
@register(pattern=r"^\.باند عام(?: |$)(.*)", sudo=True)
async def gben(userbot):
    ...
    if user:
        if user.id in DEVS:
            return await dark.edit("**Gagal Global Banned, dia adalah sang pangeran 🤴**")
        if user.id in WHITELIST:
            return await dark.edit(
                "**Gagal Global Banned, dia adalah admin @ZedThon 🤪**"
            )
        try:
        ...
        await dark.edit("**Balas Ke Pesan Penggunanya kak**")
    try:
...
@zed_cmd(pattern=r"الغاء باند عام(?: |$)(.*)")
@register(pattern=r"^\.الغاء باند عام(?: |$)(.*)", sudo=True)
async def gunben(userbot):
    ...
    if user:
        if user.id in blacklistzed:
            return await dark.edit(
                "**Gagal ungbanned, Karna dia ada di Blacklist Zee**"
            )
        try:
