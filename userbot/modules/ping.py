...
from userbot.utils import edit_or_reply, humanbytes, zed_cmd
...
async def _(ping):
    ...
    await xx.edit(
        ...
        f"▹ **بانكـر** - `%sms`\n"
        f"▹ **الزمـن -** `{uptime}` \n"
        f"**✦҈͜͡المـالك :** [{user.first_name}](tg://user?id={user.id})" % (duration)
    )


@zed_cmd(pattern="xping$")
async def _(ping):
    uptime = await get_readable_time((time.time() - StartTime))
    start = datetime.now()
