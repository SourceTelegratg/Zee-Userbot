...
from userbot.events import zed_cmd, register
...
@bot.on(zed_cmd(outgoing=True, pattern=r"رد (.*)"))
...
@bot.on(zed_cmd(outgoing=True, pattern=r"توقف (.*)"))
...
@bot.on(zed_cmd(outgoing=True, pattern=r"حذف رد (.*)"))
...
@bot.on(zed_cmd(outgoing=True, pattern=r"الردود$"))
