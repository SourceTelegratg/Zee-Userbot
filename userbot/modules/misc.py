...
from userbot.utils import edit_or_reply, zed_cmd, time_formatter
...
@zed_cmd(pattern="sleep ([0-9]+)$", allow_sudo=False)
...
@zed_cmd(pattern="shutdown$", allow_sudo=False)
async def shutdown_bot(event):
    ...
    if BOTLOG_CHATID:
        await event.client.send_message(
            BOTLOG_CHATID,
            "**#SHUTDOWN** \n"
            "**Zee-Userbot** telah di matikan!\nJika ingin menghidupkan kembali silahkan buka heroku",
        )
    await edit_or_reply(event, "**Zee-Userbot Berhasil di matikan!**")
...
@zed_cmd(pattern="restart$", allow_sudo=False)
async def restart_bot(event):
    await edit_or_reply(event, "**Zee-Userbot Berhasil di Restart**")
    if BOTLOG_CHATID:
        await event.client.send_message(
            BOTLOG_CHATID, "#RESTART \n" "**Zee-Userbot Berhasil Di Restart**"
        )
    args = [sys.executable, "-m", "userbot"]
    execle(sys.executable, *args, environ)


@zed_cmd(pattern="readme$")
async def reedme(event):
    await edit_or_reply(
        event,
        "**Berikut sesuatu untuk kamu baca:**\n"
        "\n✣ [Userbot Repo](https://github.com/Zed-Thon/Zee-Userbot/blob/Zee-Userbot/README.md)"
        "\n✣ [Video Tutorial](https://youtu.be/tTDaPKsGC1I)"
        "\n✣ [List Variabel Heroku untuk Zee-Userbot](https://telegra.ph/List-Variabel-Heroku-untuk-Zed-Userbot-09-22)"
...
@zed_cmd(pattern="repo$")
async def repo_is_here(event):
    xx = await edit_or_reply(event, "`Processing...`")
    await xx.edit(
        f"**Hey**, __I am using__ 🔥 **Zed-Userbot** 🔥\n\n"
        f"      __Thanks For Using me__\n\n"
        f"✣ **Userbot Version :** `{BOT_VER}@{branch}`\n"
        f"✣ **Group Support :** [Zed Support](t.me/ZedThon_Group)\n"
        f"✣ **Channel Zee :** [Zee Project](t.me/ZedThon)\n"
        f"✣ **Owner Repo :** [زلـزال_الهيبــه](t.me/zzzzl1l)\n"
        f"✣ **Repo :** [Zed-Userbot](https://github.com/Zed-Thon/Zee-Userbot)\n"
    )


@zed_cmd(pattern="string$")
...
CMD_HELP.update(
    {
        ...
        \n\n  •  **Syntax :** `{cmd}sleep`\
        \n  •  **Function : **Biarkan Zee-Userbot tidur selama beberapa detik \
    "
    }
)
...
CMD_HELP.update(
    {
        "repo": f"**Plugin : **`Repository Zee-Userbot`\
        \n\n  •  **Syntax :** `{cmd}repo`\
        \n  •  **Function : **Menampilan link Repository Zee-Userbot\
        \n\n  •  **Syntax :** `{cmd}string`\
        \n  •  **Function : **Menampilan link String Zee-Userbot\
    "
    }
)
...
CMD_HELP.update(
    {
        "restart": f"**Plugin : **`Restart Zee-Userbot`\
        \n\n  •  **Syntax :** `{cmd}restart`\
