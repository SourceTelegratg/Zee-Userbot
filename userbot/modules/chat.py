...
from userbot.utils import edit_delete, edit_or_reply, get_user_from_event, zed_cmd


@zed_cmd(pattern="userid$")
...
async def get_users(event):
    ...
    zed = await edit_or_reply(event, f"**Mengundang Member Dari Group {zed_}**")
    zeduserbot = await get_chatinfo(event)
    ...
    f = 0
    error = "None"
    await zed.edit("**Terminal Status**\n\n`Sedang Mengumpulkan Pengguna...`")
    async for user in event.client.iter_participants(zeduserbot.full_chat.id):
        try:
