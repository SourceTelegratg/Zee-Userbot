...
async def img_sampler(event):
    xx = await edit_or_reply(event, "**╮ ❐ جـاري البحث عن 3 صـور افتراضياً...او استخدم .صور + عدد + اسم  𓅫╰**")
...
@zed_cmd(pattern="currency ([\d\.]+) ([a-zA-Z]+) ([a-zA-Z]+)")
...
@zed_cmd(pattern="جوجل ([\s\S]*)")
async def gsearch(q_event):
    zed = await edit_or_reply(q_event, "**- جـاري البحـث ...**")
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                return await edit_delete(zed, f"**- خطـأ:**\n`{e}`", time=10)
    msg = ""
    for i in range(lim):
...
@zed_cmd(pattern="ويكي (.*)")
async def wiki(wiki_q):
    match = wiki_q.pattern_match.group(1)
    xx = await edit_or_reply(wiki_q, "** - جـاري البحـث ...**")
    try:
        summary(match)
...
async def yt_search(video_q):
    ...
    xx = await edit_or_reply(video_q, "**- جـاري البحـث ...**")
    try:
...
@zed_cmd(pattern="تحميل(صوت|فيديو( \d{0,4})?) (.*)")
...
@zed_cmd(pattern="سكرين (.*)")
async def capture(url):
    xx = await edit_or_reply(url, "**- جـاري...**")
