...
@zed_cmd(pattern="kickme$", allow_sudo=False)
...
@zed_cmd(pattern="kikme$", allow_sudo=False)
...
@zed_cmd(pattern="leaveall$", allow_sudo=False)
async def kickmeall(event):
    Zed = await edit_or_reply(event, "`Global Leave from group chats...`")
    er = 0
    done = 0
