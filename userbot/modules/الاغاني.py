...
from userbot.utils import bash, chrome, edit_or_reply, zed_cmd, progress
...
async def _(event):
    ...
    if event.pattern_match.group(1):
        ...
        xx = await edit_or_reply(event, "**╮ جـارِ البحث ؏ـن الاغنيـٓه... 🎧♥️╰**")
    elif reply.message:
        query = reply.message
    ...
    loa = glob.glob("*.mp3")[0]
    await xx.edit("**╮ ❐ جـارِ تحميـل الاغنيـٓه انتظـر قليلاً  ▬▭... 𓅫╰**")
    c_time = time.time()
...
@zed_cmd(pattern="فيديو(?: |$)(.*)")
async def _(event):
    reply = await event.get_reply_message()
    if event.pattern_match.group(1):
        query = event.pattern_match.group(1)
        xx = await edit_or_reply(event, "**╮ جـارِ البحث ؏ـن الفيديـو 🎬♥️╰**")
    elif reply:
    ...
    if l:
        await xx.edit("**╮ ❐ جـارِ تحميـل الفيديـو انتظـر قليلاً  ▬▭... 𓅫╰**")
    else:
        await xx.edit(
...
@zed_cmd(pattern="deez (.+?|) (FLAC|MP3\_320|MP3\_256|MP3\_128)")
async def _(event):
    ...
    strings = {
        ...
        "wrong_cmd_syntax": "Bruh, sekarang saya pikir seberapa jauh kita harus melangkah. tolong hentikan Sesi saya Ã°ÂÂ¥Âº",
...
CMD_HELP.update(
    {
        ...
        \n\n  â¢  **Syntax :** `{cmd}smd` <nama lagu>\
        \n  â¢  **Function : **Mendowload lagu dari bot @SpotifyMusicDownloaderBot\
        \n\n  â¢  **Syntax :** `{cmd}smd now`\
        \n  â¢  **Function : **Unduh penggunaan scrobble LastFM saat ini dari bot @SpotifyMusicDownloaderBot\
        \n\n  â¢  **Syntax :** `{cmd}net` <nama lagu>\
        \n  â¢  **Function : **Mendowload lagu dari bot @WooMaiBot\
        \n\n  â¢  **Syntax :** `{cmd}net now`\
        \n  â¢  **Function : **Unduh penggunaan scrobble LastFM saat ini dari bot @WooMaiBot\
        \n\n  â¢  **Syntax :** `{cmd}mhb` <Link Spotify/Deezer>\
        \n  â¢  **Function : **Mendowload lagu dari Spotify atau Deezer dari bot @MusicsHunterBot\
        \n\n  â¢  **Syntax :** `{cmd}deez` <link spotify/deezer> FORMAT\
        \n  â¢  **Function : **Mendowload lagu dari deezer atau spotify.\
        \n  â¢  **Format   : ** `FLAC`, `MP3_320`, `MP3_256`, `MP3_128`.\
    "
    }
)
