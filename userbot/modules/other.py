...
@zed_cmd(pattern="الرابط(?: |$)(.*)")
async def _(event):
    xx = await edit_or_reply(event, "**⇜ جـاري جلـب رابـط المجموعـه ⇜**")
    try:
        e = await event.client(
            ExportChatInviteRequest(event.chat_id),
        )
        await xx.edit(f"**❈╎رابـط الـمجموعـه : ✓**\n\n➥ {e.link}") 
    except ChatAdminRequiredError:
        return await xx.edit("**❈╎عزيزي {ALIVE_NAME} لسـت مشرفـاً في هـذه المجموعـه **")


@zed_cmd(pattern="tmsg (.*)")
async def _(event):
