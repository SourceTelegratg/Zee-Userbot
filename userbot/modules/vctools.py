...
# <https://www.github.com/Zed-Thon/Zee-Userbot/blob/main/LICENSE/>.
...
@zed_cmd(pattern="بدء مكالمه$", group_only=True)
@register(pattern=r"^\.بدء مكالمه$", sudo=True)
...
@zed_cmd(pattern="انهاء مكالمه$", group_only=True)
@register(pattern=r"^\.انهاء مكالمه$", sudo=True)
...
@zed_cmd(pattern="vcinvite", group_only=True)
async def _(c):
...
@zed_cmd(pattern="vctitle(?: |$)(.*)", group_only=True)
...
@zed_cmd(pattern="joinvc(?: |$)(.*)", group_only=True)
@register(pattern=r"^\.joinvcs(?: |$)(.*)", sudo=True)
async def _(event):
    Zed = await edit_or_reply(event, "`Processing...`")
    if len(event.text.split()) > 1:
        chat_id = event.text.split()[1]
        try:
            chat_id = await event.client.get_peer_id(int(chat_id))
        except Exception as e:
            return await Zed.edit(f"**ERROR:** `{e}`")
    else:
    ...
    if chat_id:
        ...
        try:
            ...
            return await edit_delete(
                Zed, "**INFO:** `akun anda sudah berada di obrolan suara`", 45
            )
        except Exception as e:
            return await Zed.edit(f"**INFO:** `{e}`")


@zed_cmd(pattern="leavevc(?: |$)(.*)", group_only=True)
