...
async def detect(event):
    ...
    zedevent = await edit_or_reply(event, "**MenDownload file untuk diperiksa...**")
    media = await event.client.download_media(reply)
    if not media.endswith(("png", "jpg", "webp")):
        return await edit_delete(event, "**Mohon Reply ke gambar atau stiker!**", 90)
    zedevent = await edit_or_reply(event, "**Detecting NSFW limit...**")
    r = requests.post(
    ...
    os.remove(media)
    if "status" in r.json():
        return await edit_delete(zedevent, r.json()["status"])
