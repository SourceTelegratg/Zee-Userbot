...
async def kang(args):
    ...
    if photo:
        ...
        if (
            ...
            await xx.edit("**⌔∮ جارِ التحظير لحزمة جديده...**")
            async with args.client.conversation("@Stickers") as conv:
                try:
                    await conv.send_message(cmd)
                except YouBlockedUserError:
        ...
        await xx.edit(
            ...
            f"\n        🥳🎟 **[اضغـط هنـا](t.me/addstickers/{packname})** 🎟🥳\n**- لاستخدام الملصقات**",
            parse_mode="md",
        )
        ...
        try:
            create_st = await tgbot(
                functions.stickers.CreateStickerSetRequest(
                    user_id=OWNER_ID,
                    title=pname,
                    short_name=f"zed_{un_}_V{pack}_by_{bot_un}",
                    stickers=stcrs,
                )
            )
...
@zed_cmd(pattern="معلومات الملصق$")
...
@zed_cmd(pattern="جلب ملصق$")
async def sticker_to_png(sticker):
    if not sticker.is_reply:
        await edit_delete(sticker, "**- بالــرد على ملصـق ...**")
        return False
    img = await sticker.get_reply_message()
    if not img.document:
        await edit_delete(sticker, "**- عـذراً ، هـذا ليـس ملصقـاً ؟!**")
        return False
    xx = await edit_or_reply(sticker, "**- تم جـلب الملصـق .. بنجـاح ✓**")
    image = io.BytesIO()
...
@zed_cmd(pattern="ملصقات ?([\s\S]*)")
async def cb_sticker(event):
    query = event.pattern_match.group(1)
    if not query:
        return await edit_delete(event, "**- ارسـل اسـم لـ حزمـة الملصقـات 🎟؟!**")
    ...
    if not results:
        return await edit_delete(xx, "**- اووبس .. لا يمكن العثور على حزمة الملصقات ؟!**")
    reply = f"**- حزمـة ملصقـات البحث:**\n {query}\n\n**- النتائـج:**\n"
    for pack in results:
        if pack.button:
