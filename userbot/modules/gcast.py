...
while 0 < 6:
    _GCAST_BLACKLIST = get(
        "https://raw.githubusercontent.com/Zed-Thon/Reforestation/master/blacklistgcast.json"
    )
    if _GCAST_BLACKLIST.status_code != 200:
        if 0 != 5:
            continue
        GCAST_BLACKLIST = [-1001473548283, -1001390552926, -1001234685537, -1001675396283]
        break
...
@zed_cmd(pattern="addblacklist(?:\\s|$)([\\s\\S]*)")
async def add(event):
...
@zed_cmd(pattern="delblacklist(?:\\s|$)([\\s\\S]*)")
async def _(event):
