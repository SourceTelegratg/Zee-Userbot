...
#
# Ported by @ZedThon
# FORM Zee-Userbot <https://github.com/Zed-Thon/Zee-Userbot>
...
from userbot.utils import edit_delete, edit_or_reply, zed_cmd, runcmd


@zed_cmd(pattern="convert ?(foto|audio|gif|voice|photo|mp3)? ?(.*)")
async def cevir(event):
    botzed = event.pattern_match.group(1)
    try:
        if len(botzed) < 1:
            await edit_delete(
                event,
