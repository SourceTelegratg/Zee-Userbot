...
# Support
GROUP = os.environ.get("GROUP", "ZedThon_Group")
...
WEATHER_DEFCITY = os.environ.get("WEATHER_DEFCITY", "Baghdad")
...
ALIVE_TEKS_CUSTOM = os.environ.get("ALIVE_TEKS_CUSTOM", "** بـوت  زدثــون 𝙕𝞝𝘿𝙏𝙃𝙊𝙉  يعمـل .. بنجـاح ☑️ 𓆩 **")

# Default .alive name
ALIVE_NAME = os.environ.get("ALIVE_NAME", "𝗭𝗧𝗵𝗼𝗻")
...
COUNTRY = str(os.environ.get("COUNTRY", "IQ"))
...
async def check_botlog_chatid() -> None:
    if not BOTLOG_CHATID:
        LOGS.warning(
            "var BOTLOG_CHATID kamu belum di isi. Buatlah grup telegram dan masukan bot @Rio2Robot lalu ketik /id Masukan id grup nya di var BOTLOG_CHATID"
        )
        sys.exit(1)
...
with bot:
    try:
                ...
                if user_id is not None:
                    try:
                        ...
                        return await event.reply(
                            "❌ **- تـم حظـر هـذا البـوت من قبـل المستخـدم.**"
                        )
                    except Exception as e:
        ...
        async def on_plug_in_callback_query_handler(event):
            if event.query.user_id == uid or event.query.user_id in DEVS and SUDO_USERS:
                ...
                await event.edit(
                    "⚜️ **زر وضع التعليمات مغلق!** ⚜️", buttons=openlagi
                )
            else:
                reply_pop_up_alert = f"عـذراً ، قم بتنصيب زدثــون هذا البوت خـاص بـ {owner}"
                await event.answer(reply_pop_up_alert, cache_time=0, alert=True)
    ...
    try:
        LOOP.run_until_complete(check_botlog_chatid())
