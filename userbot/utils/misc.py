...
async def animator(media, mainevent, textevent):
    ...
    Zelzal = await mainevent.client.download_media(media, TEMP_DOWNLOAD_DIRECTORY)
    await textevent.edit("**- جـاري التحويـل لـ ملصـق فيديـو ...**")
    await runcmd(
        f"ffmpeg -ss 00:00:00 -to 00:00:02.900 -i {Zelzal} -vf scale={w}:{h} -c:v libvpx-vp9 -crf 30 -b:v 560k -maxrate 560k -bufsize 256k -an Video.webm"
    )
    os.remove(Zelzal)
    vid = "Video.webm"
    return vid
