    ...
    try:
        ...
        if not args:
            if not noedits:
                await edit_delete(
                    zedevent,
                    "**- بالـرد على الرسالة أو باضافة ايدي المستخدم / معرف المستخدم!**",
                    60,
                )
            return None, None
    except Exception as e:
...
async def checking(client):
    gocheck = str(pybase64.b64decode("QFplZFRob24="))[2:10]
    checker = str(pybase64.b64decode("QHplZHRob25fZ3JvdXA="))[2:16]
    if client:
        try:
