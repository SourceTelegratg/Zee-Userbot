...
async def extract_time(man, time_val):
    if any(time_val.endswith(unit) for unit in ("s", "m", "h", "d", "w")):
        ...
        if not time_num.isdigit():
            await man.edit("المدة الزمنية المحددة غير صالحة.")
            return None
        if unit == "s":
            ...
            await man.edit(
                f"**نوع الوقت الذي تم إدخاله غير صالح. الرجاء إدخال** s أو m أو h أو d أو w ولكن لديك: `{time_val[-1]}`"
            )
            return None
        return bantime
    await man.edit(
...
async def edit_or_reply(
    ...
    if aslink or deflink:
        linktext = linktext or "**الرسالة طويلة جدًا**"
...
def post_to_telegraph(title, html_format_content):
    ...
    auth_name = "Zee-Userbot"
    auth_url = "https://github.com/Zed-Thon/Zee-Userbot"
...
async def media_to_pic(event, reply):
    ...
    if mediatype not in ["Photo", "Round Video", "Gif", "Sticker", "Video"]:
        await edit_delete(
            event,
            "**- لا يمكنني استخراج الصورة لمزيد من المعالجة...**",
        )
        return None
    ...
    if mediatype == "Sticker":
        ...
        if not os.path.exists(file):
            await edit_delete(
                event,
                f"**عذراً. لا أستطيع استخراج صورة من هذه {mediatype}**",
            )
            return None
    else:
        im = Image.open(media)
